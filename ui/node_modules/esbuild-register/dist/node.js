...
function register(esbuildOptions = {}) {
  ...
  const originalResolveFilename = _module3.default.Module._resolveFilename;
  const coreModules = new Set(_module3.default.Module.builtinModules);
  const config = _tsconfigpaths.loadConfig.call(void 0, ".");
  const matchPath = config.resultType === "failed" ? void 0 : _tsconfigpaths.createMatchPath.call(void 0, config.absoluteBaseUrl, config.paths, config.mainFields, config.addMatchAll);
  _module3.default.Module._resolveFilename = function(request, parent) {
    if (!parent || matchPath === void 0) {
  ...
  return {
    unregister() {
      revert();
      _module3.default.Module._resolveFilename = originalResolveFilename;
    }
  };
}
